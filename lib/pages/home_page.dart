...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> _loadSyncedHealthData() async {
    try {
      ...
      double stepSum = 0; int stepCount = 0;
      for (int i = 1; i <= 7; i++) {
      ...
      if (activeMin == 0 && activeAvg == 0) { actLabel = "기록 없음"; actColor = Colors.grey; }
      else if (activeMin >= activeAvg) { actLabel = "평소보다 많음"; actColor = Colors.green; }
      else { actLabel = "평소보다 적음"; actColor = Colors.orange; }
      ...
      int lastHr = 0;
      if (hrData.isNotEmpty) lastHr = (_numVal(hrData.first.value) ?? 0).round();
      ...
      } else { hrLabel = "기록 없음"; hrColor = Colors.grey; }

      final recovery = await _loadTodayRecoveryScore(today0);
      int recScore = recovery?.score ?? 0;
      String recLabel = "분석 중"; Color recColor = Colors.grey;
      if (recovery != null) {
        ...
        child: ListView(
          ...
          children: [
            ...
            GridView.count(
              ...
              children: [
                _HealthCard(
                  title: '수면 점수', value: _sleepScore.toString(), unit: '점', icon: Icons.bedtime, iconColor: Colors.indigoAccent,
                  statusColor: _sleepStatusColor, statusLabel: _sleepStatusLabel, onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const SleepDetailPage())),
                ),
                _HealthCard(
                  title: '심박수', value: _heartRate.toString(), unit: 'bpm', icon: Icons.favorite, iconColor: Colors.redAccent,
                  statusColor: _hrStatusColor, statusLabel: _hrStatusLabel, onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const HeartRateDetailPage())),
                ),
                _HealthCard(
                  title: '회복 점수', value: _recoveryScore.toString(), unit: '점', icon: Icons.bolt, iconColor: Colors.purpleAccent,
                  statusColor: _recoveryStatusColor, statusLabel: _recoveryStatusLabel, onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const StressRecoveryPage())),
                ),
                _HealthCard(
                  title: '활동 시간', value: _todayActivityMin < 60 ? "$_todayActivityMin" : "${_todayActivityMin ~/ 60}",
                  unit: _todayActivityMin < 60 ? "분" : "시간 ${_todayActivityMin % 60}분", icon: Icons.directions_run, iconColor: Colors.orange,
                  statusColor: _activityStatusColor, statusLabel: _activityStatusLabel, onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const StepsPage())),
                ),
              ],
            ),
            ...
            _EnvironmentFullCard(
              temp: temp, humidity: humidity, co2: co2, pm1: pm1, pm25: pm25, pm10: pm10, odor: odor, airQuality: airQual,
            ),
            const SizedBox(height: 32),

            _SectionHeader(title: '모드 변경'),
            Container(
              padding: const EdgeInsets.all(6), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(20), boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.05), blurRadius: 10, offset: const Offset(0, 4))]),
              child: Row(children: [
...
// ✅ [상태 로직 함수] 사진 기준: 파랑(좋음), 초록(보통), 노랑(나쁨), 빨강(매우나쁨)
// 참고: 삼성 에어모니터 플러스 기준
({Color color, String label}) _getAirStatus(String type, double val) {
  if (type == 'PM10') {
    // 30이하 좋음, 80이하 보통, 150이하 나쁨, 그 이상 매우나쁨
    ...
    // 15이하 좋음, 35이하 보통, 75이하 나쁨, 그 이상 매우나쁨
    ...
    // 1000이하 좋음, 1500이하 보통, 3000이하 나쁨, 그 이상 매우나쁨
    ...
    // 1:좋음(파랑), 2:보통(초록), 3:나쁨(노랑), 4:매우나쁨(빨강)
    int level = val.toInt();
...
class _MenuCard extends StatelessWidget { final String title; final IconData icon; final Color color; final VoidCallback onTap; const _MenuCard({required this.title, required this.icon, required this.color, required this.onTap}); @override Widget build(BuildContext context) => InkWell(onTap: onTap, borderRadius: BorderRadius.circular(20), child: Container(decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(20), boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.03), blurRadius: 6)]), child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: color.withOpacity(0.1), shape: BoxShape.circle), child: Icon(icon, color: color, size: 28)), const SizedBox(height: 8), Text(title, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: Colors.black87))]))); }
